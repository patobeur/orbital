...
class Physic {
	...
	check_collisions = (obj, typeobj) => {
		...
		this.MF[typeobj].forEach((objB) => {
			if (objB.immat != obj.immat) {
				...
				distance - alertedistancebeforedie <
				obj.sizwhl.w / 2 + objB.sizwhl.w / 2
	...
	check_collisionsDirectives = (obj, objB) => {
		if (obj.collide.collideself && objB.objtype != "fruits") {
	...
	set_NewNicePosition_broken = (obj) => {
		...
		if (
			(ratioDir > 0.9375 && ratioDir <= 1) ||
			(ratioDir >= 0 && ratioDir <= 0.0625)
		) {
			obj.direction.compass = "N";
			obj.posxyz.y -= velocityY;
		} else if (ratioDir > 0.0625 && ratioDir <= 0.1875) {
			obj.direction.compass = "NE";
			obj.posxyz.x += velocityX / 2;
			obj.posxyz.y -= velocityY / 2;
		} else if (ratioDir > 0.1875 && ratioDir <= 0.3125) {
			obj.direction.compass = "E";
			obj.posxyz.x += velocityX;
		} else if (ratioDir > 0.3125 && ratioDir <= 0.4375) {
			obj.direction.compass = "SE";
			obj.posxyz.x += velocityX / 2;
			obj.posxyz.y += velocityY / 2;
		} else if (ratioDir > 0.4375 && ratioDir <= 0.5625) {
			obj.direction.compass = "S";
			obj.posxyz.y += velocityY;
		} else if (ratioDir > 0.5625 && ratioDir <= 0.6875) {
			obj.direction.compass = "SW";
			obj.posxyz.x -= velocityX / 2;
			obj.posxyz.y += velocityY / 2;
		} else if (ratioDir > 0.6875 && ratioDir <= 0.8125) {
			obj.direction.compass = "W";
			obj.posxyz.x -= velocityX;
	...
	getNextPos = (obj) => {
		...
		let y = obj.posxyz.y;
		let nxX =
			x * Math.cos(obj.direction.deg) - y * Math.sin(obj.direction.deg);
		let nxY =
			x * Math.sin(obj.direction.deg) + y * Math.cos(obj.direction.deg);
		obj.posxyz.x = nxX;
		obj.posxyz.y = nxY;
	};
	...
	get_NextOrbitPos = (obj) => {
		...
		if (obj.parentimmat) {
			const center = this.MF.sobs.find(
				(sob) => sob.immat === obj.parentimmat[0]
			);
	...
	check_IsPosOutScreen(obj) {
		if (obj.posxyz.x > this.DM.IniDatas.cosmosSize.w) {
			obj.posxyz.x = 1 - obj.sizwhl.w;
		}
		if (obj.posxyz.x <= 0 - obj.sizwhl.w) {
			obj.posxyz.x = this.DM.IniDatas.cosmosSize.w;
		}
		if (obj.posxyz.y <= 0 - obj.sizwhl.h) {
			obj.posxyz.y = this.DM.IniDatas.cosmosSize.h;
		}
		if (obj.posxyz.y > this.DM.IniDatas.cosmosSize.h + obj.sizwhl.h) {
			obj.posxyz.y = 1;
		}
	}
}
