...
class MobFactory {
    ...
    add_obj = (objdatas) => {
        let obj = {
            ...
            status: { immune: ((objdatas.status && objdatas.status.immune) ? objdatas.status.immune : false) },
            statusdelay: { immune: [0, 0] },
            ...
            ranges: objdatas.ranges ?? { social: { d: 4 * ((objdatas.sizwhl.w + objdatas.sizwhl.h) / 2) } },
            collide: { collidesocial: false, colliderangea: false, collideself: false, collidealert: false },
            contact: objdatas.contact ? { social: objdatas.contact.social ?? false, exchange: objdatas.contact.exchange ?? false } : false
        };
    ...
    create_EveryBasics() {
		...
		this.add_obj({
			...
			parentimmat: false,
			direction: { ratio: 0, degZ: 0, deg: aleaEntreBornes(1, 360), delay: 400, currentdelay: 0, way: [0, 0, 0, 0], compass: '', agility: 45 },
			contact: { social: [], exchange: [] }
		})
		this.add_obj({
			div: 'mob', // mob is mobile or sob is static
			...
			parentimmat: false,
			direction: { ratio: 0, degZ: 0, deg: aleaEntreBornes(1, 360), delay: 100, currentdelay: 0, way: [0, 0, 0, 0], compass: '', agility: 22.5 },
			contact: { exchange: [] }
		})
