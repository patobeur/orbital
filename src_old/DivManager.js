...
class DivManager {
	...
	cssMaker = () => {
		...
		stringcss +=
			".center {position: absolute;width: 2px;height: 2px;background-color: rgba(0, 0, 0, .5);}";
		stringcss +=
			"#pause {position: absolute;bottom: 20%;left: 50%;width: max-content;height: max-content;transform: translate(-50%, -50%);background-color: rgba(153, 205, 50, 0.3);color: white;border-radius: 0.5rem;padding: 0.5rem;font-size: 1.5rem;display: none;}";
	...
	get_IniDatas = () => {
		...
		return {
			...
			cosmosSize: {
				w: lunarzone.width - screenborder.w * 2,
				h: lunarzone.height - screenborder.h * 2,
				l: lunarzone.width - screenborder.l * 2,
			},
		};
	};
	...
	createEle = (attrib = false) => {
		...
		if (attrib) {
			...
			attrib.borderRadius
				? (tag.style.borderRadius = attrib.borderRadius)
				: "";
			...
			attrib.backgroundColor
				? (tag.style.backgroundColor = attrib.backgroundColor)
				: "";
			attrib.color ? (tag.style.color = attrib.color) : "";

			attrib.display ? (tag.style.display = attrib.display) : "";
			attrib.flexDirection
				? (tag.style.flexDirection = attrib.flexDirection)
				: "";
			attrib.justifyContent
				? (tag.style.justifyContent = attrib.justifyContent)
				: "";
	...
	appendChild_Bonus() {
		...
		let allbonus = this.createEle({
			tag: "div",
			id: "statusbonus",
			className: "statusbonus",
		});
		// immune bonus social
		let range = this.createEle({
			tag: "div",
			id: "visualsocial",
			className: "range",
		});
		let content = this.createEle({
			tag: "div",
			id: "visualsocialcontent",
			className: "content",
			textContent: "👩‍🚀",
		});
		let count = this.createEle({
			tag: "div",
		...
		// immune bonus visual
		range = this.createEle({
			tag: "div",
			id: "visualimmune",
			className: "range",
		});
		content = this.createEle({
			tag: "div",
			id: "visualimmunecontent",
			className: "content",
			textContent: "💎",
		});
		count = this.createEle({
			tag: "div",
	...
	get_centerPos = (poss, type = { x: 0, y: 0, z: 0 }) => {
		...
		if (typexy.x === "right") {
			xyz.x = this.IniDatas.cosmosSize.w - poss.w;
		}
		if (typexy.y === "top") {
			xyz.y = 0;
		}
		if (typexy.y === "bottom") {
			xyz.y = this.IniDatas.cosmosSize.h - poss.h - 50;
		}
		// console.log(xyz)
		return xyz;
	};
	...
	redrawAllMobs(allMobs) {
		allMobs.forEach((obj) => {
			...
			if (currentMob) {
				...
				let divrange = document.getElementById(
					"rangea" + obj.div + "-" + obj.immat
				);
				// if (divrange) {
				// 	if (obj.objtype === 'player') {
				// 	}
				...
				if (obj.objtype === "player") {
					...
					if (obj.status.immune && obj.statusdelay.immune[0] > 0) {
						...
						visualimmunecount.textContent =
							obj.statusdelay.immune[1] - obj.statusdelay.immune[0]; //(parseInt((obj.statusdelay.immune[0] * 100)) / 1000)
					} else {
				...
				errors.push([
					"this.redrawAllMobs",
					"can't target " + obj.objname + obj.div + "-" + obj.immat,
				]);
			}
			// }
		});
